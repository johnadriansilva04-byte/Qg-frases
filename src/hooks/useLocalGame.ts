...
export function useLocalGame(difficulty: Difficulty, human: Player = 1): LocalGame {
  ...
  const commit = useCallback((move: Move) => {
    setState((cur) => {
      ...
      if (formed.length > 0 && move.remove === null) {
        ...
        // Decrementa a mão aqui para não permitir colocar mais de 9 peças
        // enquanto a captura não é escolhida.
        if (move.from === null) {
  ...
  // Durante a captura, o estado parcial já tem a peça que fechou a trilha posta.
  // Reconstruímos o tabuleiro pós-movimento para garantir que os alvos sejam
  // apenas peças INIMIGAS (nunca as do próprio jogador).
