...
export function Sidebar() {
  ...
  return (
    <>
      {/* Botão no extremo esquerdo para abrir sidebar */}
      {!isOpen && (
        <button
      ...
      {/* Sidebar retrátil (abre no lado esquerdo, 30% de altura) */}
      <div
        className={`fixed left-0 top-0 w-80 bg-surface/50 border-r border-b border-border z-50 transition-all duration-300 ease-in-out ${
          isOpen ? "h-[30vh]" : "h-0"
        } overflow-hidden`}
      >
        {/* Conteúdo da sidebar */}
        <div className="flex flex-col h-full">
          ...
          {/* Lista de notícias */}
          <div className="flex-1 overflow-y-auto p-4 space-y-4 max-h-[calc(30vh-60px)]">
            {loading ? (
