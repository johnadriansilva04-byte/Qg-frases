...
import { Newspaper, X, ExternalLink, Clock, ChevronDown } from "lucide-react";
...
export function Sidebar() {
  ...
  return (
    <>
      {/* Botão no topo para abrir sidebar */}
      {!isOpen && (
        <button
          onClick={() => setIsOpen(true)}
          className="fixed top-4 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-full bg-primary text-primary-foreground shadow-lg hover:bg-primary/90 transition-all hover:scale-105 flex items-center gap-2"
      ...
      {/* Sidebar retrátil (abre de cima para baixo) */}
      <div
        className={`fixed left-0 right-0 top-0 bg-surface/50 border-b border-border z-50 transition-all duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="flex flex-col h-full">
          ...
          {/* Lista de notícias */}
          <div className="flex-1 overflow-y-auto p-4 space-y-4 max-h-[calc(80vh-60px)]">
            {loading ? (
