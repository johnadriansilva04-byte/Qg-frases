...
function Piece({
  ...
  return (
    <g
      ...
      className={`origin-center transition-transform duration-200 ${clickable ? "cursor-pointer hover:scale-110" : ""}`}
    ...
    >
      ...
      {isSelected && <circle r="5.8" fill="none" stroke="#fbbf24" strokeWidth="0.9" />}
      <ellipse cx="0.5" cy="1.3" rx="4.2" ry="3.6" fill="rgba(0,0,0,0.45)" />
      <circle
        r="4.1"
        fill={feb ? "#3b82f6" : "#ef4444"}
        stroke={inMill ? "#22c55e" : "rgba(0,0,0,0.8)"}
        strokeWidth="0.7"
      />
      {feb ? (
        // Cobra fumando estilizada
        <g fill="none" stroke="#ffffff" strokeWidth="0.55" strokeLinecap="round">
...
export function TrilhaBoard({
  ...
  return (
    <div className="relative aspect-square w-full max-w-[min(78vh,640px)]">
      <div className="absolute inset-0 rounded-md bg-surface/50 border border-border" />
      <svg
        viewBox="0 0 100 100"
      ...
      >
        ...
        <g stroke="rgba(0,0,0,0.1)" strokeWidth="0.3">
        ...
        {/* linhas do tabuleiro */}
        <g stroke="rgba(0,0,0,0.8)" strokeWidth="1.1" strokeLinecap="round">
          {BOARD_EDGES.map(([a, b]) => {
        ...
        <g stroke="#22c55e" strokeLinecap="round">
          {mills.map((m) => {
        ...
        {NODE_COORDS.map(([gx, gy], node) => {
          ...
          return (
            <g key={`n${node}`} transform={`translate(${px(gx)} ${px(gy)})`}>
              <circle r="1.6" fill="rgba(0,0,0,0.7)" />
              {isTarget && (
                <circle
                  r="3.4"
