...
export function TrilhaOnlineGame({ mesaId, onBack, onFinish }: TrilhaOnlineGameProps) {
  ...
  const captureTargets = useMemo(() => {
    ...
    // Com a captura pendente, o turno permanece com quem fechou a trilha
    // (o servidor não troca o turno até a peça ser removida). A vítima é o
    // adversário de quem está capturando — nunca as próprias peças.
    const foe = opponent(mesa.turn);
    const targets = removableTargets(gameState.board, foe);

    // Defesa extra: jamais oferecer peça própria como alvo de captura.
    return new Set(targets.filter((node) => gameState.board[node] === foe));
  }, [mesa]);
  ...
  const handleNodeClick = async (node: number) => {
    ...
    if (mesa.phase === "placing") {
      // Regra das 9 peças: sem reserva não se coloca mais nenhuma peça.
      const minhaReserva =
        myPlayerNumber === 1 ? mesa.hand_p1 : mesa.hand_p2;
      if (minhaReserva <= 0) return;
      if (targets.has(node)) {
  ...
  const fazerJogada = async (from: number | null, to: number, remove: number | null) => {
    ...
    // Completar uma captura pendente repete o movimento que fechou a trilha:
    // a peça já está posta no tabuleiro e a mão já foi debitada — então a
    // repetição é idempotente (não decrementa mão de novo, não reaplica o movimento).
    const completingCapture = mesa.pending_capture && remove !== null;
    ...
    if (formedMill && remove === null && !mesa.pending_capture) {
