 ...
 *
 * Persistência: Supabase (tour_modulos_visitados em botao_usuarios).
 * Fallback: localStorage quando offline.
 */
...
interface Props {
  modulo: TourModule;
  userId: string | null;
  /** Módulos já visitados (vindos do Supabase via BotaoGame). */
  visitados?: string[];
  /** Callback para salvar módulo visitado no Supabase. */
  onMarcarVisitado?: (modulo: string) => void;
}

/**
 * PRACINHA GUIDE — tour contextual por módulo.
 *
 * 1. Ao montar, verifica se `modulo` já foi visitado (props visitados do Supabase).
 * 2. Se não foi → mostra o tour (passos com overlay de destaque).
 * 3. Ao finalizar → chama onMarcarVisitado e desaparece.
 * 4. Se não tem userId (offline), usa localStorage como fallback.
 */
export function PracinhaGuide({ modulo, userId, visitados = [], onMarcarVisitado }: Props) {
  ...
  const localKey = "pracinha:visitedModules";
  const getLocalVisitados = (): string[] => {
    try {
      const raw = localStorage.getItem(localKey);
      return raw ? JSON.parse(raw) : [];
    } catch { return []; }
  };
  const setLocalVisitado = (mod: string) => {
    try {
      const atual = getLocalVisitados();
      if (!atual.includes(mod)) {
        localStorage.setItem(localKey, JSON.stringify([...atual, mod]));
      }
    } catch { /* quota */ }
  };
  ...
  useEffect(() => {
    const todosVisitados = userId ? visitados : getLocalVisitados();
    if (!todosVisitados.includes(modulo) && passos.length > 0) {
      setTourAtivo(true);
      setIndice(0);
    }
  }, [modulo, passos.length, userId, visitados]);
  ...
  const avancar = useCallback(() => {
    ...
    if (proximo >= passos.length) {
      ...
      if (userId && onMarcarVisitado) {
        onMarcarVisitado(modulo);
      } else {
        setLocalVisitado(modulo);
      }
      setTourAtivo(false);
      setDestacar(null);
      return;
  ...
  }, [indice, passos, modulo, userId, onMarcarVisitado]);
  ...
  const pular = useCallback(() => {
    ...
    setTourAtivo(false);
    setDestacar(null);
  }, [modulo, userId, onMarcarVisitado]);
