...
-- Migration separada para criar apenas salas (assumindo botao_usuarios já existe)

-- Tabela de salas (lobby)
CREATE TABLE IF NOT EXISTS public.botao_salas (
...
CREATE INDEX IF NOT EXISTS idx_botao_salas_status ON public.botao_salas(status);
CREATE INDEX IF NOT EXISTS idx_botao_salas_jogador1 ON public.botao_salas(jogador1_session);
CREATE INDEX IF NOT EXISTS idx_botao_salas_jogador2 ON public.botao_salas(jogador2_session);
CREATE INDEX IF NOT EXISTS idx_botao_salas_created_at ON public.botao_salas(created_at DESC);
...
DROP POLICY IF EXISTS "Todos podem ver salas" ON public.botao_salas;
CREATE POLICY "Todos podem ver salas" ON public.botao_salas FOR SELECT USING (true);

DROP POLICY IF EXISTS "Autenticados podem criar salas" ON public.botao_salas;
CREATE POLICY "Autenticados podem criar salas" ON public.botao_salas FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Autenticados podem atualizar salas" ON public.botao_salas;
CREATE POLICY "Autenticados podem atualizar salas" ON public.botao_salas FOR UPDATE USING (true);

DROP POLICY IF EXISTS "Autenticados podem deletar salas" ON public.botao_salas;
